...
def test_scc_speedup_vs_networkx(exponent: int) -> None:
    ...
    print(f"\n  SCC 10^{exponent}: nx={nx_time:.4f}s | cgraph={c_time:.4f}s ({speedup:.1f}x)")  # noqa: T201
...
def test_wcc_speedup_vs_networkx(exponent: int) -> None:
    ...
    speedup = nx_time / c_time if c_time > 0 else float("inf")
    print(f"\n  WCC 10^{exponent}: nx={nx_time:.4f}s | cgraph={c_time:.4f}s ({speedup:.1f}x)")  # noqa: T201
...
def test_toposort_speedup_vs_networkx(exponent: int) -> None:
    ...
    speedup = nx_time / c_time if c_time > 0 else float("inf")
    print(f"\n  toposort 10^{exponent}: nx={nx_time:.4f}s | cgraph={c_time:.4f}s ({speedup:.1f}x)")  # noqa: T201
